 ...
 * is created so they are cached locally. They should be refreshed manually by a call to {@link #refresh()}
 * when it is believed those attributes have changed.</p>
 * 
...
public class RemoteFile extends File {
	...
	/** File path, preferably absolute. */
	protected String pathname;
	
	/** The JRF client used to request file information. */
	...
	public void refresh() throws IOException {
		MsgFileInfos infos = getFileInfos();
		length = infos.length();
	...
	public static RemoteFile[] listRoots(JRFClient server) {
		...
		try {
			...
			FileInfos[] rfiles = ((MsgFileList)msg).getFiles();
	...
	private long getLong(Message msgLong) {
		...
		try {
			...
			if (!(msg instanceof MsgFileLong))
				throw new IOException("Unexpected reply message "+msg);
			ex = null;
			return ((MsgFileLong)msg).getValue();
		} catch (IOException e) {
			ex = e;
			return 0l;
		}
	...
	public RemoteFile[] listFiles() {
		...
		try {
			...
			if (!(msg instanceof MsgFileList))
				throw new IOException();
			FileInfos[] infos = ((MsgFileList)msg).getFiles();
			if (infos == null)
				return null;
	...
	public MsgFileInfos getFileInfos() throws IOException {
		...
		Message msg = cli.getReply(num, 0);
		cli.addLatencyNow(t0);
		if (!(msg instanceof MsgFileInfos))
			throw new IOException();
		return (MsgFileInfos)msg;
	}
